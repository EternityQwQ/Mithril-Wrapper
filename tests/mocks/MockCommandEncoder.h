...
public:
    ...
    core::Result setScissor(const backend::ScissorRect&) override { calls.emplace_back("scissor"); return {}; }
    core::Result setCullState(backend::CullMode, backend::FrontFace) override { calls.emplace_back("cull"); return {}; }
    core::Result setBlendColor(float, float, float, float) override { calls.emplace_back("blendColor"); return {}; }
    ...
    core::Result bindBuffer(backend::RenderStage, std::uint32_t, core::BufferHandle, std::size_t) override { calls.emplace_back("shaderBuffer"); return {}; }
    core::Result bindBytes(backend::RenderStage, std::uint32_t, std::span<const std::byte>) override { calls.emplace_back("bytes"); return {}; }
    core::Result bindTexture(backend::RenderStage, std::uint32_t, std::uint32_t, core::TextureHandle, core::SamplerHandle) override { calls.emplace_back("texture"); return {}; }
